...
async def create_deposit_session(user_id: int, amount: float) -> str:
    """Создаёт сессию пополнения и возвращает уникальную метку"""
    import uuid
    from config import DEPOSIT_NOTE_PREFIX
    
    note = f"{DEPOSIT_NOTE_PREFIX}{user_id}_{uuid.uuid4().hex[:8]}"
    expires_at = datetime.now() + timedelta(hours=1)
    
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT OR REPLACE INTO deposit_sessions (user_id, amount, expires_at) VALUES (?, ?, ?)",
...
async def get_deposit_session(user_id: int):
    """Получает активную сессию пополнения"""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM deposit_sessions WHERE user_id = ? AND expires_at > CURRENT_TIMESTAMP",
            (user_id,)
        ) as cur:
            return await cur.fetchone()


async def clear_deposit_session(user_id: int):
    """Очищает сессию пополнения"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "DELETE FROM deposit_sessions WHERE user_id = ?",
            (user_id,)
        )
        await db.commit()


async def create_deposit(user_id: int, amount: float, tx_hash: str, note: str) -> int:
    """Создаёт запись о депозите"""
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            """INSERT INTO deposits (user_id, amount, tx_hash, note, status, processed_at)
               VALUES (?, ?, ?, ?, 'completed', CURRENT_TIMESTAMP)""",
            (user_id, amount, tx_hash, note)
        )
        await db.commit()
        return cur.lastrowid


async def get_pending_deposit_by_note(note: str):
    """Ищет депозит по метке (для проверки дубликатов)"""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM deposits WHERE note = ?",
            (note,)
        ) as cur:
            return await cur.fetchone()


async def get_user_deposits(user_id: int, limit: int = 10):
