...
async def show_profile_content(callback: CallbackQuery, lang: str):
    """Показывает профиль пользователя"""
    user = await get_user(callback.from_user.id)
    deposits = await get_user_deposits(callback.from_user.id, 5)
    ref_count = await get_referrals(callback.from_user.id)
...
async def show_profile_callback(callback: CallbackQuery, lang: str):
    await show_profile_content(callback, lang)
